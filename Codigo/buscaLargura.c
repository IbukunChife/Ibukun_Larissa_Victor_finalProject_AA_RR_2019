...
int buscaLargura(int vertice, int *caminho, int **grafo, int **fluxo){
    int A_origem = vertice+1; //determino que a A-origem e B-destino são o penultimo e ultimo elementos
    int B_destino = vertice+2;
    fila *f = criaFila(); //uso a função de criar fila que me retorna uma fila do tipo fila.

    int *visitado; // 0 é não-visitado, 1 é visitado, porém não todos os vizinhos, 2 é visitado todos os vizinhos.
    ...
    enfileirar(A_origem-1, f, visitado); //enfileiro o vértice origem na fila, no caso a A-origem,e ele torna visitado (1) no vetor visitado.
    ...
    return (check == 2); //retorna 2 se achar um caminho até B-destino, ou seja, chegou no B-destino.
}
